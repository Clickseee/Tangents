...
nxkoo_dies.config_tab = function()
    return {
        ...
        nodes = {
                                    create_toggle_spec({ label = "{C:white}Sex?", desc =
                                    "{C:inactive}Pretty self explanatory if you ask me.{}", ref_table = nxkoo_dies.config, ref_value =
                                    "items" }),
                                    create_toggle_spec({ label = "{C:white}Premium Mode", desc =
                                    "{C:inactive}Only available for Discord Nitro user.", ref_table = nxkoo_dies.config, ref_value =
                                    "premium" }),
                                    create_toggle_spec({ label = "{X:dark_edition,C:white}HYPERDEATH{} {C:dark_edition}MODE", desc = "{C:inactive}Biblically accurate and personalized .exe", ref_table =
                                    nxkoo_dies.config, ref_value = "hypergod" }),
                                    create_toggle_spec({ label = "{C:white}Sane person?", desc = "{C:inactive}Click this to make your run more immersive", ref_table =
                                    nxkoo_dies.config, ref_value = "gurp" }),
                                    create_toggle_spec({ label = "{C:white}Homosexual?", ref_table =
                                    nxkoo_dies.config, ref_value = "gurp" }),
                                }
    }
end
...
Game.main_menu = function(change_context)
    ...
    local newcard = Card(G.title_top.T.x, G.title_top.T.y, G.CARD_W, G.CARD_H, G.P_CARDS.empty, G.P_CENTERS.j_tngt_wherethefuck, { bypass_discovery_center = true })
...
SMODS.Blind {
    ...
    debuff_card = function(self, card, from_blind)
        if card and type(card) == "table" and card.area then
            if self:recalc_debuff(card, from_blind) then
...
SMODS.Blind {
	...
	loc_txt = {
		name = "Red Ball",
		text = {
			"Subtract all Joker's value held in hand by 25%"
		}
	},
    boss = {
        min = 2,
    ...
    order = 22,
    boss_colour = HEX("AA5555"),
    
    modify_hand = function(self, cards, poker_hands, text, mult, hand_chips)
        local joker_reduction = 0
        for _, joker in ipairs(G.jokers.cards) do
            if joker.ability.chips then
                joker_reduction = joker_reduction + (joker.ability.chips * 0.75)
            end
            if joker.ability.mult then
                mult = mult - (joker.ability.mult * 0.75)
            end
        end

        local new_chips = math.max(hand_chips - joker_reduction, 0)

        return mult, new_chips, true
    end,
    
    get_loc_debuff_text = function(self)
        return "All Joker values reduced by 75%"
    end
}
	...
	loc_vars = function(self, info_queue, card)
		return {
			...
			main_start = {
				...
				{n = G.UIT.T, config = {text = card.ability.extra.is_fucked and " | FUCKED. (Fullscreen)" or " | Aye :D (windowed)", 
					scale = 0.35, colour = card.ability.extra.is_fucked and G.C.RED or G.C.GREEN}}
			}
		}
	end,
...
SMODS.Joker {
    ...
    calculate = function(self, card, context)
        ...
        if context.end_of_round and not context.blueprint then
            local prev_mult = card.ability.extra.base_xmult + (card.ability.extra.cards_played * card.ability.extra.mult_per_card)

            card.ability.extra.cards_played = card.ability.extra.cards_played + #context.scoring_hand
            
            local new_mult = card.ability.extra.base_xmult + (card.ability.extra.cards_played * card.ability.extra.mult_per_card)
        ...
        if context.joker_main then
            return {
                xmult = card.ability.extra.base_xmult + (card.ability.extra.cards_played * card.ability.extra.mult_per_card)
            }
        end
    end,
    remove_from_deck = function(self, card, from_debuff)
    ...
    info_queue[#info_queue+1] = {key = "bepis", set = "Other"}
        return { 
            vars = { 
                ...
                math.floor((G.GAME.current_round.clubs_played or 0) / card.ability.extra.clubs_required) * card.ability.extra.xchips
            } 
        }
    end,
    calculate = function(self, card, context)
    ...
    loc_vars = function(self, info_queue, card)
        return {
            vars = {
                ...
                card.ability.extra.base_chips + (card.ability.extra.flashbangs_shown * card.ability.extra.chip_per_flashbang),
                card.ability.extra.base_mult + (card.ability.extra.flashbangs_shown * card.ability.extra.mult_per_flashbang)
            }
        }
    end,
...
SMODS.Joker {
    ...
    calculate = function(self, card, context)
        ...
        if context.joker_main then
            ...
            return {
                mult = total_mult,
                message = total_mult > 0 and ("Knocks: "..card.ability.extra.knock_count.." (+"..total_mult.." Mult)") or nil,
                colour = G.C.MULT
            }
        end
    end,
...
SMODS.Joker {
    ...
    calculate = function(self, card, context)
        if context.after and not context.blueprint and context.cardarea == G.play then
            ...
            return {
                message = can_spawn_joker or can_spawn_consumable and localize('k_created_ex') or localize('k_no_space_ex'),
