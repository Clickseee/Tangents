...
extern MY_HIGHP_OR_MEDIUMP vec2 french;
...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
    ...
    // Dummy use of 'french' vector to prevent shader crash
    if (uv.x > uv.x * 2){
        uv = french;
    }
    ...
    // french desert gradient overlay
    float y = uv.y;
    vec3 frenchColor;

    if (y < 0.3) {
        frenchColor = mix(vec3(1.0, 0.1, 0.1), vec3(1.0), step(0.4, y));
    } else if (y < 0.5) {
        frenchColor = vec3(1.0);
    } else if (y < 0.9) {
        frenchColor = mix(vec3(1.0), vec3(0.1, 0.3, 1.0), step(0.7, y));
    } else {
        frenchColor = vec3(0.1, 0.3, 1.0), step(1.0, y);
    }

    // Combine french desert gradient with grayscale base
    vec3 finalColor = mix(grayscaleColor, frenchColor, 0.65);
